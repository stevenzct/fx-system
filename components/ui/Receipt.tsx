...
export function Receipt({ transaction, className = "" }: Readonly<{ transaction: Transaction; className?: string }>) {
  return (
    <article className={`print-receipt rounded-2xl border border-line bg-white p-5 font-mono text-slate-950 ${className}`}>
      <header className="mb-3 border-b border-dashed border-slate-400 pb-3 text-center">
      ...
      <p className="mt-5 text-center text-[11px] text-slate-500">Posted to FX Ledger - System Generated Receipt</p>
    </article>
  );
}
...
function ReceiptLine({ label, value, strong }: Readonly<{ label: string; value: string; strong?: boolean }>) {
  return (
    <div className={`flex justify-between gap-4 py-1.5 text-xs ${strong ? "mt-2 border-t border-dashed border-slate-400 pt-3 font-black" : ""}`}>
      <span>{label}</span>
